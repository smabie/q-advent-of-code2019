...
pos:(0N; 3)#("J"$) each last flip init
...
step:{(p+v; v:last[x]+sum each (0N; count p)#gravity p:first x)}
...
single:{(p+v; v:last[x]+sum each (0N; count p)#adj each p cross p:first x)}
